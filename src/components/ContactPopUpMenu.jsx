...
export default function ContactPopUpMenu({ isActivePopup, setIsActivePopup, isActivePopupValue }) {
  ...
  const handleDelete = async (id) => {
    try {
      const deleteData = await fetch(`${BaseUrl}/inquiry/deleteInquiry?id=${id}`, {
        method: "DELETE",
        headers: {
    ...
    <>
      <div className={`${style.backdrop} ${isClosing ? style.fadeOut : style.fadeIn}`}>
        <div className={`${style.containerDiv} ${isClosing ? style.close : style.open}`}>
          ...
          <p className={style.confirmationText}>{`Are you sure you want to delete contact?`}</p>
